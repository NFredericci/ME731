...
data_longo %>%
  ...
  labs(
    title = "MC)dia das Medidas MorfolC3gicas ao Longo do PerC-odo",
    x = "PerC-odo",
    y = "MC)dia"
  ) +
  theme_minimal() +
...
# AnC!lise sem Data Imputing nas medidas, apenas retirando os NAs das medidas
...
autovalores
cat("\nNumero de fatores pelo Criterio de Kaiser:", fatores_kaiser, "\n")

# AF

round(cor(medidas), 2)

factanal(x = medidas, factors = 8) # nao converge e o p-valor < 0.05; vou descartar

af_resultado_psych <- fa(
  r = medidas,
  nfactors = 2,
  rotate = "promax", 
  fm = "ml", # maxima verossimilhanca
  scores = "regression")

plot(af_resultado_psych)
...
rg.plot <- apply(af_resultado_psych$scores, 2, range)
rg.raw <- apply(af_resultado_psych$loadings, 2, range)
temp <- sweep(af_resultado_psych$loadings, 2, rg.plot[2,]/rg.raw[2,], "*")

plot(af_resultado_psych$scores[,1], af_resultado_psych$scores[,2], 
     col = Periodo_vetor, 
     pch = 19,
     xlab = "Fator 1 (Tamanho)", 
     ylab = "Fator 2 (Forma/ProporC'C#o Residual)",
     main = "Mapa de Escores Fatoriais (FA com psych::fa)")

arrows(0, 0, temp[,1], temp[,2], 
       length = 0.1, 
       col = "red",
       lwd = 1.5)
...
### quais pares diferem? (nada foi significativo, coloquei o teste para demonstrar)
...
df_medidas$Cluster_Kmeans <- modelo_kmeans$cluster

medias_kmeans <- df_medidas %>%
  group_by(Cluster_Kmeans) %>%
  summarise(across(everything(), mean, na.rm = TRUE)) 
print("MC)dias das Medidas por Cluster K-means:")
print(medias_kmeans)
...
grupos_hclust <- cutree(modelo_hclust, k = 2)

fviz_cluster(list(data = dados_cluster_scaled, cluster = grupos_hclust),
             ellipse.type = "convex",
             palette = "jco",
             ggtheme = theme_minimal(),
             main = "HCLUST (K=2) Clustering")

df_medidas$Cluster_Hclust <- grupos_hclust

medias_hclust <- df_medidas %>%
  group_by(Cluster_Hclust) %>%
  summarise(across(everything(), mean, na.rm = TRUE))

medias_hclust

fviz_nbclust(dados_cluster_scaled, pam, method = "silhouette") 
...
plot(modelo_pam, main = paste("PAM Clustering (K =", K_escolhido_pam, ") - GrC!fico de Silhueta"))

fviz_cluster(modelo_pam, data = dados_cluster_scaled,
             ellipse.type = "convex",
             palette = "set1", 
             ggtheme = theme_minimal(),
             main = paste("PAM Clustering (K =", K_escolhido_pam, ")"))
...
medias_hclust <- medias_hclust %>% select(-c(Cluster_Kmeans, Cluster_PAM))
...
ggplot(
  ...
  geom_point(size = 3) +
  labs(
    title = "GrC!fico de Perfil dos Clusters de Cranios (Todas as 22 Medidas)",
    subtitle = "Cluster 1 (Caes Maiores) vs. Cluster 1 (Caes Menores)",
    x = "Medida do Cranio",
...
medias_kmeans <- medias_kmeans %>% select(-c(Cluster_Hclust, Cluster_PAM))
...
medias_pam <- medias_pam %>% select(-c(Cluster_Hclust, Cluster_Kmeans))
...
ggplot(
  ...
  geom_point(size = 3) +
  labs(
    title = "Grafico de Perfil dos Clusters de Cranios (PAM, K=2)",
    subtitle = "Comparacao das medias entre os dois Medoides (Cranios Tipicos)",
    x = "Medida do Cranio (Escalonada)",
    y = "Media da Medida (Centroide)") +
  theme_minimal(base_size = 14) +
  theme(
    plot.title = element_text(hjust = 0.5, face = "bold"),
